"""
TradeOS India — Groq (Llama 3.1 70B) LLM client.

Uses llama-3.1-70b-versatile via the groq SDK. API key loaded from
OS keyring. Preferred for overnight research and high-volume tasks
due to low cost (free tier).
"""
...
_KEYRING_KEY = "tradeOS_groq_key"
_COST_PER_1K_INR = 0.05
_MAX_RETRIES = 3
_INITIAL_BACKOFF_S = 1.0


class GroqClient(BaseLLM):
    """Groq Llama 3.1 70B LLM client.

    Low-cost provider ideal for overnight research and high-volume tasks.
    Loads API key from OS keyring, retries on transient errors.
    """
    ...
    def _ensure_client(self) -> bool:
        ...
        if not self._api_key:
            log.warning("Groq API key not found in keyring")
            return False
        ...
        try:
            ...
            log.error("groq SDK not installed")
            return False
        except Exception as exc:
            log.error(f"Failed to init Groq client: {exc}")
            return False
    ...
    async def complete(
        ...
        for attempt in range(1, _MAX_RETRIES + 1):
            try:
                ...
                log.debug(
                    f"Groq complete: {tokens_in}+{tokens_out} tokens, "
                    f"{duration_ms}ms, INR {cost_inr:.4f}"
                )
                return text or ""

            except groq.RateLimitError as exc:
                log.warning(
                    f"Groq rate limit (attempt {attempt}/{_MAX_RETRIES}): {exc}"
                )
                ...
                log.error(
                    f"Groq API error (attempt {attempt}/{_MAX_RETRIES}): {exc}"
                )
                if attempt < _MAX_RETRIES:
                    await asyncio.sleep(backoff)
                    backoff *= 2
                ...
                log.error(f"Groq unexpected error: {exc}")
    ...
    async def stream(
        ...
        try:
            ...
            log.warning(f"Groq stream rate limit: {exc}")
            yield '{"error": "Rate limit"}'
        except groq.APIError as exc:
            log.error(f"Groq stream API error: {exc}")
            yield f'{{"error": "{str(exc)[:100]}"}}'
        except Exception as exc:
            log.error(f"Groq stream error: {exc}")
            yield f'{{"error": "{str(exc)[:100]}"}}'
